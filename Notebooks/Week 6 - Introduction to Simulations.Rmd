...
#### Full Points
For full points you need to code a function that determines this probability.  It is okay if you use the same multiples of 13 rule.  A graph is not required but nice to have.

    if ((aCard %% 13) == 0) {         # Cards 13,26,39,52 are aces (all of these are modulus %% of 13)
      foundAce <- TRUE
    } 

```{r DrawingAces, echo=FALSE, message=FALSE}
...
```{r CoinTossSimulation, echo=FALSE, message=FALSE}
...
Same, but with a rbinom statement like this:  rbinom(1,size=1,prob=0.5)


```{r CoinTossUsingrbinom, echo=FALSE, message=FALSE}
datalist = list()
for (i in 1:50) {
...
HINT: YOU MUST USE THIS FORMULA

**(factorial(n)/(factorial(x[i])*factorial(n-x[i])))*(p^x[i])*(1-p)^(n-x[i])**


#### Full Points
You may use the same mathematical forumula as well as the dbinom() function. 
```{r DiscreteDistribution, echo=FALSE, message=FALSE}
...
```{r ProbabilityandDistributionwithQuantile}
...
HINT:  One easy way is to use the scale(x) function

#### Full Points
Write a function to calcuate z-scores.
```{r ZScore, echo=FALSE, message=FALSE}
set.seed(1234)
